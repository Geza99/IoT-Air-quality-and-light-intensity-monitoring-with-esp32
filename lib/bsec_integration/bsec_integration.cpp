                    ...
                    bme680_com_fptr_t bus_read, sleep_fct sleep, state_load_fct state_load, config_load_fct config_load)
{
    ...
    {       
        ...
        if (ret.bsec_status != BSEC_OK)
        {
            Serial.println("Failed setting config!");
            return ret;
        }
        Serial.println("Configuration set succesfully.");
    }
    ...
    {       
        ...
        if (ret.bsec_status != BSEC_OK)
        {
            Serial.println("Error setting state!");
            return ret;
        }
        else
        {
            Serial.println("State set sucessfully");
        }
    }
...
static void bme680_bsec_trigger_measurement(bsec_bme_settings_t *sensor_settings, sleep_fct sleep)
{
    ...
    int8_t bme680_status = BME680_E_INVALID_LENGTH;
    ...
    if (sensor_settings->trigger_measurement)
    {
        ...
        {
            Serial.println("BSEC ERROR! bme680_set_sensor_settings");
        }

        bme680_status = BME680_E_INVALID_LENGTH;   
        /* Set power mode as forced mode and trigger forced mode measurement */
        bme680_status = bme680_set_sensor_mode(&bme680_g);
        
        if (bme680_status != BME680_OK)
        {
            Serial.println("BSEC ERROR! bme680_set_sensor_mode");
        }
    ...
    int32_t bsec_process_data)
{
    ...
    if (bsec_process_data)
    {
        ...
        if(bme680_status != BME680_OK)
        {
            Serial.println("BME ERROR! bme680_get_sensor_data");
        }
...
static void bme680_bsec_process_data(bsec_input_t *bsec_inputs, uint8_t num_bsec_inputs, output_ready_fct output_ready)
{
    ...
    bsec_library_return_t bsec_status = BSEC_E_DOSTEPS_INVALIDINPUT;
    ...
    if (num_bsec_inputs > 0)
    {
        ...
        {
            Serial.println("BSEC ERROR! bsec_do_steps");
        }
...
/*!
 * @brief       Function for getting data from BME680 and passing it  to BSEC library
 *
 * @param[in]   sleep               pointer to the system specific sleep function
 * @param[in]   time_stamp          timestamp
 * @param[in]   output_ready        pointer to the function processing obtained BSEC outputs
 * @param[in]   state_save          pointer to the system-specific state save function
 * @param[in]   save                flag indicating whether BSEC state should be saved
 * @param[in]   get_timestamp_us    pointer to the system-specific timestamp derivation function
 *
 * @return      none
 */
void bsec_iot_op(sleep_fct sleep, uint64_t time_stamp, output_ready_fct output_ready,
                    state_save_fct state_save, uint8_t save, get_timestamp_us_fct get_timestamp_us)
{
    ...
    if (bsec_status != BSEC_OK)
    {
        Serial.print("ERROR getting sensor settings! ERR: ");
        Serial.println(bsec_status);
    }
    ...
    bsec_status = BSEC_E_DOSTEPS_INVALIDINPUT;
    ...
    if (save != 0)
    {
        ...
        {
            Serial.println("Error retrieving state!");
        }
    }
}
...
{
    int8_t bme680_status = BME680_E_INVALID_LENGTH;
    bme680_status = bme680_get_regs(0xD0, &chip_id, 1, &bme680_g);
    if(bme680_status != BME680_OK)
    {
        Serial.println("BME ERROR! bme680_get_sensor_data");
    }
    ...
    {
        Serial.printf("BME680 chip ID: %d.\r\n",chip_id);
    } 
}
